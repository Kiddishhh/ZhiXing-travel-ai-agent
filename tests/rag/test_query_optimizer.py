...
class TestQueryOptimizeResult:
    ...
    def test_default_values(self):
        _print_stage("QueryOptimizeResult 数据模型", 3, 1)
    ...
    def test_multi_query_result(self):
        _print_stage("QueryOptimizeResult 数据模型", 3, 2)
        result = QueryOptimizeResult(
    ...
    def test_hyde_result(self):
        _print_stage("QueryOptimizeResult 数据模型", 3, 3)
        result = QueryOptimizeResult(
...
class TestQueryOptimizer:
    ...
    def test_optimize_empty_query(self):
        _print_stage("QueryOptimizer 边界", 3, 1)
        optimizer = QueryOptimizer()
    ...
    def test_optimize_fallback_on_error(self):
        ...
        _print_stage("QueryOptimizer 容错", 3, 2)
        print("[注入] LLM 调用抛出 RuntimeError('模拟 LLM 故障')")
